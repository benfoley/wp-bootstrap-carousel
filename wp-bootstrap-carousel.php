<?php
/**
 * Plugin Name: CBMM Bootstrap Carousel 
 * Plugin URI: 
...
class WP_Bootstrap_Carousel
{
    ...
    public function shortcode( $atts )
    {
        ...
        foreach( $query as $item_id => $item )
        {
            ...
            $thumb   = wp_get_attachment_image_src( $item->ID, $image_size, false ); // thumbnail size
            $link    = ( $file ) ? $full[0] : get_attachment_link( $item->ID );
            $caption = ( ! empty( $item->post_excerpt ) ) ? wpautop( wptexturize( $item->post_excerpt ) ) : '';

            // open item div
            $carousel .= '<div id="item-' . $item->ID . '" class="' . ( ( $i == 0 ) ? "active" : "" ) . ' item">';
            ...
            // comments
            /*
            if( $comments && comments_open( $item->ID ) )
            {
                $comments = ( get_comments_number( $item->ID ) != 0 ) ? sprintf( _n( '1 Comment', '%1$s Comments', get_comments_number( $item->ID ), 'wp_bootstrap_carousel' ), number_format_i18n( get_comments_number( $item->ID ) ) ) : __( '0 Comments', 'wp_bootstrap_carousel' );

                $carousel .= '<p class="carousel-comments-link">
                    <a href="' . get_comments_link( $item->ID ) . '" rel="bookmark">' . $comments . '</a>
                </p>';
            }
            */
             ...
             * @param   integer $item->ID   The attachment ID
             */
            $carousel .= apply_filters( 'wp_bootstrap_carousel_caption_text', $caption, $item->ID );
...
<a class="right carousel-control" href="#wp-bootstrap-carousel-' . $post_parent . '" role="button" data-slide="next">
    ...
    public function get_data( $atts )
    {
        ...
        $atts = shortcode_atts( apply_filters( 'wp_bootstrap_carousel_shortcode_atts', array(
            ...
            'ids'           => '',      // Allow gallery image IDs to be used
            'order'             => 'ASC',
            'orderby'           => 'ID',
            ...
            'thickbox'          => 0,       // (bool) Whether thickbox should be used.
        ...
        $include        = array_map( 'intval', explode( ',', $atts['ids'] ) );
        ...
        if ( is_array($include) ) :
            $query      = get_posts( compact( 'include', 'post_status', 'post_type', 'post_mime_type', 'exclude', 'order', 'orderby' ) );
        else :
